...
The Docker image sets `FREESTYLE_DB_PATH`, `HOST`, and `PORT` for you. `FREESTYLE_DB_PATH` is required — the server refuses to start without it (the image defaults it to `/data/freestyle.db`).
